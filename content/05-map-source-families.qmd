...
AI can generate candidate source families when the researcher supplies a concept, period, place, and actor set. Ask for institutional record types, vernacular sources, retrospective accounts, and material or visual traces. Require the model to state why each family might exist and why it might not.

Every named archive, collection, database, or repository must be verified through an official catalogue or institutional source. Fabricated repository names are a predictable failure. Record them as rejected suggestions rather than deleting them from the audit trail.

## Integrity Checkpoint

Do not place sensitive source locations, participant identities, or restricted collection details into an external model. Access conditions are part of the evidence map, but public disclosure can harm people and violate agreements. Use abstract descriptions when the operational detail is protected.

## Save the Artifact

Save the source-family inventory with one row per family and links to the concept identifiers it may serve. Add fields for repository, access status, language, temporal coverage, likely dependence on other families, and known silences.
...
A parallel discipline has developed where the source family arrives as a digitized collection. The collections-as-data movement asks libraries and archives to publish collections as research datasets with documented provenance, rights, processing steps, and known gaps.^[See @padilla2023vancouver.] Its instruments are practical. Datasheets adapted from machine learning describe how heritage material passed through survival, collection, digitization, and text recognition, each stage adding its own selection.^[See @alkemade2023datasheets.] Checklists specify what an institution should document before releasing a collection for computational use, and what a researcher should document before deriving a dataset or training a model on one.^[See @candela2025checklist and @lee2025mlchecklist.] When a source family arrives as a downloadable corpus, ask for its datasheet. Where none exists, the researcher inherits the documentation burden, and the source-family inventory is the place to carry it.

...
This manuscript now tests the method through two complete modules. The first moves from a question to an evidence map and collection strategy. The second turns the scholarly source family into a literature corpus, synthesis, coverage audit, and stopping rule. Both use the same page pattern, preserve consequential decisions, and assign AI bounded roles.

The draft does not yet cover every stage imagined in the original project. Field collection, post-production, coding, analysis, preservation, and public communication require later modules. The two completed modules establish a repeatable architecture and show that it can support more than one kind of research task.

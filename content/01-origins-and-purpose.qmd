...
Two papers recorded the durable argument beneath those drafts. The first distinguished structured data from unstructured sources and argued that political research needs both. Structure permits systematic comparison, but it reduces context; unstructured material preserves sequence, attribution, and meaning, but demands disciplined retrieval and interpretation.[@grossman2020bigdata] The second moved earlier in the research cycle. It described the paradox of digital abundance and proposed a networked research memory that could connect sources, excerpts, annotations, concepts, and projects without confusing personal knowledge management with reference management or archiving.[@grossman2022digitalsources] The papers did not yet have today's language-model infrastructure. They supplied the problem definition and the research discipline that infrastructure must now serve.
...
This project is a full, bilingual living field guide published in reviewed modules. English is the source edition during development; Hebrew will be a governed parallel edition with stable chapter identifiers, terminology review, citation parity, and right-to-left testing. The book is written for students completing an entire historically grounded qualitative research cycle with large, heterogeneous source collections. The current manuscript contains the first two complete modules. One moves from a question to an evidence map and collection plan. The other turns the scholarly source family into a literature corpus, synthesis, coverage audit, and stopping rule. Both use the same page pattern, preserve consequential decisions, and assign AI bounded roles.

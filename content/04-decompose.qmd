...
Provide the framed question and the research purpose. Ask the model to separate concepts, contexts, actors, mechanisms, rival explanations, and negative cases. Require it to label each suggestion as a candidate rather than a fact.
...
The researcher then checks whether each candidate follows from the question and whether it can guide evidence collection. Broad social categories proposed without a plausible evidentiary route should not enter the grid merely because they sound inclusive.

## Integrity Checkpoint

Concepts can reproduce official categories that the study should examine. They can also impose academic language that participants would reject. Record whose vocabulary each term represents. Where translation is involved, preserve the original expression and the decisions behind the English gloss.

## Save the Artifact

Save the concept-and-context grid with stable identifiers for each row. These identifiers will connect later search terms, source families, and evidence records to the dimension they serve.

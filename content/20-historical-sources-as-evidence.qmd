...
This method adds the historian's discipline to the book's existing chain. Large unstructured collections can reveal temporal sequence, institutional change, causal mechanisms, and alternative explanations that disappear when evidence is reduced too early to rows and variables.[@grossman2020bigdata] Yet digital abundance also makes it harder to retain the route among repositories, source files, excerpts, concepts, and evolving interpretations. A networked research memory supplies that continuity without replacing close reading or archival expertise.[@grossman2022digitalsources] The evidence map identifies source families. The event-database chapter converts verified records into resolved events and variables. Historical research requires closer control over the transformations that precede a verified record and the interpretations that follow it. The chapter supplies that control.
...
An extended Historical Evidence Chain companion is maintained with the project source. It includes the full research-design article, a Salem-specific codebook, a parallel human coding form, versioned transcription, extraction, and audit prompts, controlled vocabularies, and Airtable and REDCap specifications. It is an implementation lab rather than a claim of completed findings. Its public release remains pending until its separate reuse license is recorded.

...
The working branch also contains **Review Articles and Meta-Analysis in Transition**, which connects the literature workflow to contemporary evidence synthesis and defines reusable research skills, bounded agents, human gates, and versioned project context. **Building Event Databases with AI** is a second bridge chapter on large-scale source discovery, candidate-record verification, event resolution, dynamic codebooks, validation, and explicit Bayesian updating. **Historical Sources as Evidence** adds the historian's evidence chain from source witness and diplomatic transcription to source-bound observation, declared interpretation, Historical Claim, and downstream audit. **From Notes to Research Infrastructure** uses a read-only audit of Ami Pedahzur's Readwise library to trace the movement from networked notes to persistent context, bounded agents, and a seven-stage PKM-AI research loop. **Writing by Voice, Revising by Ear** adds an auditable path from AI dictation to verified prose and from multilingual text-to-speech to location-coded revision notes. The next revision program adds an early chapter on epistemic judgment and limits, moves AI integrity before the first guided workflow, carries a composite oral-history case through the method, supplies complete artifact specimens, and adds the Collection as Evidence module. See [ROADMAP.md](ROADMAP.md).

The current 26-chapter structure is also available as a working [Hebrew table of contents](docs/hebrew-table-of-contents.md). It will remain synchronized with the English-first living edition while the full Hebrew edition is developed.
